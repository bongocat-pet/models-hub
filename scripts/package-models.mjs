...
const r2OutputRoot = resolve(projectRoot, 'r2-assets/models');
...
for (const repository of catalog.repositories) {
  ...
  for (const model of catalog.models.filter(item => item.repositoryKey === repository.key)) {
    ...
    const r2Package = resolve(r2OutputRoot, model.repositoryKey, model.downloadFilename);
    await mkdir(resolve(r2OutputRoot, model.repositoryKey), { recursive: true });
    await run('git', [
